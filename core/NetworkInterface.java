...
/**
 * NetworkInterface管理一系列连接，还包含一些传输属性的信息(理解成物理层，传输距离、传输速度等)
...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	...
	public void setHost(DTNHost host) {
		...
		if (transmitRange > 0) {
			//根据不同的接口类型(hashcode),创建不同的网格单元(说白了就是配置里的interfacetype只要字符串不同,hashcode就不同)
			optimizer = ConnectivityGrid.ConnectivityGridFactory(
					this.interfacetype.hashCode(), transmitRange);
			//每个接口都有其主机对应的坐标，该坐标属于某个网格单元，此接口被添加进某个网格单元
