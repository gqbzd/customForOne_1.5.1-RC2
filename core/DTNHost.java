...
/**
 * #DTNHost和NetworkInterface，类似于IP地址+端口号#
 * A DTN capable host.
 */
public class DTNHost implements Comparable<DTNHost> {
	...
	private String name;//配置文件里取的名字
	//监听器用于统计仿真结果，当节点变化时（如：节点移动或消息创建、转发、丢弃、删除），会触发相应的监听器更新仿真结果
	...
	public void update(boolean simulateConnections) {
		//判断节点是否处于活动状态，若不是，断开与该节点建立的所有连接
		if (!isRadioActive()) {
			// Make sure inactive nodes don't have connections
			tearDownAllConnections();
			return;
		}
		//判断是否需要更新网络层(更新所有接口之间的连接)
