...
public abstract class MessageRouter {
	...
	public int receiveMessage(Message m, DTNHost from) {
		...
		this.putToIncomingBuffer(newMessage, from);	//将消息移至incomingMessages	
...
//		isFinalRecipient = aMessage.getTo() == this.host;//消息传递到目的节点
		//自定义，由结点本身来判断自己是否是目的结点
		isFinalRecipient = false;
		int thisHostAddress = this.host.getAddress();
		if(MessageEventGenerator.toHostRange[0] <= thisHostAddress && 
				thisHostAddress < MessageEventGenerator.toHostRange[1]) {
		...
		if (!isFinalRecipient && outgoing!=null) {
			...
			//自定义，将收到的消息添加进world中的计数器里
			World.singleCountDeliveredMessages.add(aMessage.getId());
			//自定义，接收到信息，打印信息内容
