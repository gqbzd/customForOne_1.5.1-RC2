...
public abstract class ActiveRouter extends MessageRouter {
	...
	/**
	 * 在本路由类的此方法,可以想象成处于网络层,它会调用物理层(Connection)的startTransfer
	...
	protected int startTransfer(Message m, Connection con) {
		...
		if (retVal == RCV_OK) { // started transfer
			...
			this.deleteMessage(m.getId(), false);//如果是目的结点，便将本结点的消息从缓存中删去
		}
	...
	/** 
	 * 对创建消息的结点来说，丢掉旧消息，为新创建的消息腾出空间。
	 * 对中继消息的结点来说，丢掉旧消息，为即将接收的消息腾出空间。
	 * 总之要保证充分的空闲空间
	...
	/**
	 * 先看消息的目的结点是否是本节点的某个邻居节点，如果没有，再查看邻居节点是否有消息的目的节点是本节点
